...
# Step 3: Generate Music with a Repeating Pattern
def generate_music_with_pattern(model, start_sequence, int_to_note, pattern_length=10, repeat_count=5, temperature=0.5, min_note='C4', max_note='C5'):
    generated_pattern = []
    ...
    for _ in range(pattern_length):
        ...
        generated_pattern.append(predicted_note)
    ...
    # Repeat the pattern to create the final sequence
    generated_notes = generated_pattern * repeat_count

    return generated_notes

# Step 4: Convert generated notes to a MIDI file
def create_midi(generated_notes, output_file='generated_music_pattern.mid'):
...
if __name__ == "__main__":
    ...
    output_midi_file = 'generated_music_result/generated_music_pattern.mid'
    pattern_length = 10  # Length of the generated pattern
    repeat_count = 5  # How many times to repeat the pattern
    ...
    # Step 9: Generate music with a repeating pattern
    start_sequence = X[np.random.randint(0, len(X))]  # Random seed from training data
    generated_notes = generate_music_with_pattern(model, start_sequence, int_to_note, pattern_length, repeat_count, temperature)
